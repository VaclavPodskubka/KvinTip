...
export default function Betting() {
  ...
  const formatMatchTime = (matchTime: string) => {
    return new Date(matchTime).toLocaleString('cs-CZ', { day: 'numeric', month: 'numeric', hour: '2-digit', minute: '2-digit' })
  }
  ...
  const settleEvent = async (eventId: string, result: 'a' | 'draw' | 'b') => {
    ...
    for (const bet of eventBets) {
      ...
      if (!player) continue
      if (won) {
        await updateDoc(doc(db, 'users', bet.userId), { credits: (player.credits ?? 0) + Math.round(bet.amount * bet.odds) })
  ...
  const placeBet = async (event: BettingEvent) => {
    ...
    await addDoc(collection(db, 'bets'), { eventId: event.id, userId: user!.uid, pick, amount, odds, status: 'pending', createdAt: new Date() })
  ...
  const filteredEvents = events.filter(e => view === 'upcoming' ? e.status !== 'finished' : e.status === 'finished')
  ...
  return (
    <main className="min-h-screen p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6 md:mb-8">
        ...
        {isAdmin && (
          <button onClick={() => setShowForm(!showForm)}
            className="font-bold py-2 px-4 md:py-2.5 md:px-6 rounded-xl text-sm transition-all duration-200"
            style={{
      ...
      {/* Admin formulář — grid 1 col na mobilu */}
      {isAdmin && showForm && (
        <div className="mb-6 p-4 md:p-6" style={glass}>
          <h2 className="font-bold text-base mb-4" style={{ color: 'white' }}>Nový zápas</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            <div>
              <p className="text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Sport</p>
            ...
            <div>
              <p className="text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Datum a čas</p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <p className="text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Tým A</p>
              <input type="text" placeholder="Man City" value={form.teamA}
                onChange={e => setForm(f => ({ ...f, teamA: e.target.value }))}
                className="w-full rounded-xl px-3 py-2 text-sm"
                style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'white' }} />
            </div>
            <div>
              <p className="text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Tým B</p>
              <input type="text" placeholder="Arsenal" value={form.teamB}
          ...
          <div className="mb-3">
            <p className="text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Volitelný název</p>
            <input type="text" placeholder="Premier League · Kolo 38" value={form.title}
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            {oddsFields.map(({ label, key }) => (
              <div key={key}>
                <p className="text-xs font-semibold mb-1.5 uppercase tracking-wider truncate" style={{ color: 'rgba(255,255,255,0.4)' }}>{label}</p>
                <input type="number" step="0.01" min="1" value={form[key] as number}
                  onChange={e => setForm(f => ({ ...f, [key]: Number(e.target.value) }))}
                  className="w-full rounded-xl px-2 py-2 text-sm text-center font-bold"
                  style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'white' }} />
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(false)}
              className="font-medium py-2.5 px-4 rounded-xl text-sm"
      ...
      <div className="flex flex-col gap-3 md:gap-4">
        {filteredEvents.map((event) => {
          ...
          return (
            <div key={event.id} className="p-4 md:p-5" style={cardStyle}>

              {/* ── ŘÁDEK 1: Sport + Status ── */}
              ...
              {/* ── ŘÁDEK 2: Název + čas ── */}
              <div className="mb-3">
                <h3 className="text-base md:text-lg font-black leading-tight" style={{ color: 'white' }}>{event.title}</h3>
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.35)' }}>📅 {formatMatchTime(event.matchTime)}</p>
                  {event.status === 'open' && (
                    <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.3)' }}>· ⏱ {timeUntilMatch(event.matchTime)}</p>
                  )}
                  <p style={{ fontSize: '0.72rem', color: '#c084fc' }}>{eventBets.length} sázek · {totalPool} kr</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-1.5 md:gap-2 mb-3">
                {(['a', 'draw', 'b'] as const).map(pick => {
                  ...
                  return (
                    <button key={pick}
                      onClick={() => { if (event.status === 'open' && !myBet) setBetPicks(prev => ({ ...prev, [event.id]: pick })) }}
                      disabled={isLocked || !!myBet}
                      className="rounded-xl p-2.5 md:p-3 text-center transition-all duration-200"
                      style={{
                      ...
                      <p className="text-xs font-bold truncate mb-1" style={{ color: 'rgba(255,255,255,0.6)' }}>{label}</p>
                      <p className="text-xl md:text-2xl font-black" style={{ color: isResult ? '#4ade80' : isSelected || myPick ? '#e9d5ff' : '#a855f7' }}>
                        {odds.toFixed(2)}
                      </p>
                      {myPick && <p style={{ fontSize: '0.6rem', color: '#93c5fd', marginTop: '0.2rem' }}>✓ Tvoje</p>}
                      {isResult && <p style={{ fontSize: '0.6rem', color: '#4ade80', marginTop: '0.2rem' }}>🏆 Výsledek</p>}
                    </button>
                  )
                })}
              </div>
                ...
                <div className="rounded-xl p-3 mb-3"
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', marginBottom: '0.2rem' }}>Tvoje sázka</p>
              ...
              {event.status === 'open' && !myBet && (
                <div className="flex gap-2">
                  <input type="number" min={1} placeholder="Kolik kreditů?"
                  ...
                  <button onClick={() => placeBet(event)} disabled={submitting === event.id}
                    ...
                    style={{
                      ...
                      color: 'white', opacity: submitting === event.id ? 0.5 : 1,
              ...
              {isAdmin && event.status !== 'finished' && (
                <div className="mt-3 pt-3 flex flex-col gap-2" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  <p style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                    ⚙️ Admin — vyhodnotit
                  </p>
                  <div className="grid grid-cols-3 gap-1.5">
                    <button onClick={() => settleEvent(event.id, 'a')}
                      className="text-xs font-bold py-2 rounded-xl transition-all truncate px-2"
